...
└── test/
    ...
    ├── verifyMermaidStyle.mjs  # mermaid stylesheet-failure robustness: stamped attrs survive removing every <style> (8 cases)
...
| `npm run verify-mermaidstyle` | Headless mermaid stylesheet-failure test: stamped attrs exist, `stroke="none"` placeholders overwritten, and computed styles survive removal of EVERY `<style>` element (8 cases, needs Playwright) |
