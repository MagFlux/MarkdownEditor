...
└── test/
    ...
    ├── verifyMermaidStyle.mjs  # mermaid stylesheet-failure robustness + font consistency (10 cases)
...
| `npm run verify-mermaidstyle` | Headless mermaid stylesheet-failure test: stamped attrs exist, `stroke="none"` placeholders overwritten, computed styles survive removal of EVERY `<style>` element, and the sheet paints the same fontFamily mermaid measures with (10 cases, needs Playwright) |
