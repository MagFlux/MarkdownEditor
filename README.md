...
- **Word-wise caret moves** — `Ctrl+Left` / `Ctrl+Right` jump the caret a whole word in the arrow's direction; add `Shift` (`Ctrl+Shift+Left` / `Ctrl+Shift+Right`) to extend/shrink the selection a whole word at a time **from the caret** — a forward selection shrinks from its right edge, and crossing the anchor flips the growth direction, like native shift+arrows. The moves are Markdown-aware and engine-independent: a word is a whitespace-delimited run — trailing punctuation (`formatting.`) rides along, a standalone ` - ` is its own stop — and a formatted span (`**bold**`, `` `code` ``, `[link](url)`, …) is atomic, markers included, so `Ctrl+Right` from `A| **lightweight**` lands at `A **lightweight**|`. At a line end the move crosses the newline and stops after the next line's first word (`step|\n- **Diagrams**` → `step\n-| **Diagrams**`)
...
- **Block formatting**: H1–H3, quotes, `ul` / `ol`, code fences, GFM tables, a one-click **Mermaid diagram** scaffold (`` ```mermaid\n\n``` `` with the caret inside), and a one-click **math block** scaffold (`$$\n\n$$` with the caret on the blank middle line, ready for LaTeX source). These inserters are **insert-only** — a second click inserts another scaffold/fence, it never removes existing content.
- **LaTeX math (KaTeX)** — `$$…$$` display blocks and inline `$…$` render as typeset math in the live preview (synchronously — no flicker), in the HTML export (with the KaTeX stylesheet and fonts **embedded as data: URIs**, so the exported file renders offline), and in the PDF export. `\$` escapes a literal dollar, math inside code fences / inline code stays literal, and prose like `costs $5 and $10` is never mis-parsed (Pandoc-style delimiter rules). Invalid LaTeX renders inline in red.
...
│   ├── math.js                 # LaTeX math ($$ block + $ inline, KaTeX): renderMarkdown pipeline (protect→parse→restore), scanner skips code regions, export-CSS builder
│   ├── katexExportAssets.js    # browser-only katex ?raw/?url imports (main.js only) for the self-contained HTML math export
│   ├── icons.js                # inline-SVG toolbar icons (B I S code link table mermaid math + save/open + new-tab + undo/redo + hamburger/file-doc + theme + view-mode glyphs; the hamburger export menu itself is text-only)
...
└── test/
    ...
    ├── verifyMermaidStyle.mjs  # mermaid stylesheet-failure robustness + label centering (38 cases)
    ├── verifyMermaidFade.mjs   # mermaid dark-theme gradient-outline fade off + no drop shadow (16 cases)
...
- **Caret placement after editor actions** — 81 assertions that every edit committed lands its caret where a typist expects it, never as a highlighted selection. Coverage: Enter on a nested bullet/ordered item continues the marker **with the parent's indentation** (a skipped-tab regression) and the empty-item Enter still exits the list; Tab indents a whole line *and blank lines* with the caret's column preserved and always a **collapsed** selection (same for Shift+Tab, including the column-0 case); inline bold over a mid-word caret or a selection collapses the caret at the end of the inner text right **before the closing `**`** (`bold **test**` with caret after the final `t`), toggle-OFF lands after the unwrapped span, and the caret-in-a-gap case still inserts the empty marker pair while keeping the caret between the markers; code-fence and ```mermaid fence wraps put the caret **inside** the fence — on the blank middle line for an empty block (````` ```\n|\n``` ```` / ````` ```mermaid\n|\n``` ````), after the content before the `\n``` ` tail otherwise; the **insert-only contract** is pinned: a table click inside an existing table inserts a second scaffold (never removes) and a non-empty caret line keeps its prose with the scaffold pushed below it, and a codeblock click with the caret on a fence line re-wraps it (no unwrap); table insert leaves the caret in the first body cell; and the h1 button leaves the caret at the end of the rewritten block.
...
| `npm test` | Round-trip renderer invariant + the math-pipeline delimiter/guard cases (Node) |
...
| `npm run verify-math` | Headless KaTeX math test: preview renders `$$` display + `$` inline math synchronously; code regions / prose dollars / blank scaffold stay literal; invalid LaTeX paints red; HTML export embeds the self-contained font-inlined katex stylesheet only when math is present; PDF export rasterizes math (21 cases, needs Playwright) |
...
| `npm run verify-mermaidstyle` | Headless mermaid stylesheet-failure test: stamped attrs exist, `stroke="none"` placeholders overwritten, computed styles survive removal of EVERY `<style>` element, the sheet paints the same fontFamily mermaid measures with, oversized label boxes get flex-recentered, plain-text labels painting with the start-anchor-from-center fingerprint (center displaced by half their width from the nearest shape) re-anchored to text-anchor:middle via the painted start-anchor fingerprint, middle-anchored labels center-snapped with a rounded translate (WebKitGTK dominant-baseline quirk) — but a MULTI-LINE label (`<br/>`-split sibling lines sharing one shape, e.g. a sequence note) is never snapped, fixing the "note text jumbled" overprint — oversized EDGE-label foreignObjects (the Windows "grey box around Ctrl+S / Export" report) normalized to the painted content on ANY >2px deviation (the oversize varys per label — mild and width-only cases covered) with half-delta x/y re-centering — tight FOs untouched, idempotent, non-edge labels never touched — and the sequence diagram's inline `url(#…-drop-shadow)` filter attributes stripped (feDropShadow markup bypasses the dropShadow themeVariable) while `-drop-shadow-small` and non-sequence SVGs stay untouched (38 cases, needs Playwright) |
| `npm run verify-mermaidfade` | Headless mermaid dark-theme gradient-outline test: no `linearGradient` in the defs, the sheet's neo node-stroke rule is not `url(...)`, computed + stamped node strokes are SOLID colors in both themes, the neo rule carries `filter: none` (no drop shadow), and the dark-theme stroke luminance stays high (16 cases, needs Playwright) |
...
| `npm run verify-caret` | Headless caret-placement test (87 cases): Enter list-continuation keeps indentation with a collapsed caret; Tab/Shift+Tab (incl. blank lines) commit without highlighting and the caret column follows the text; inline formats collapse at the end of the inner span before the closing marker; code-fence and ```mermaid fence wraps put the caret inside the fence; math-block insert caret on the blank middle line of `$$\n\n$$` (empty line replaced in place, prose kept with the block below); table insert caret in the first body cell; insert-only pins (table/codeblock/mermaid/math never remove); Markdown-aware Ctrl+Arrow word moves — whitespace-run words (trailing punctuation rides along, standalone ` - ` its own stop) with formatted spans atomic, line-crossing stops at the next line's first word, only document edges fall through native; Ctrl+Shift+Arrow gestures act on the caret edge (shrink/flip like native shift+arrows) |
